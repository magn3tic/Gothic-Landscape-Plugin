<?php
/**
 * Selections Error in Record Notification Email
 *
 ...
 * @abstract
 */
final class ErrorInRecordNotification extends MessageAbstract implements MessageInterface {
	 ...
	 * @var string
	 */
	public static $key = 'error-in-record-notification';
	...
	protected static function recipients( array $args = [] ) {
		...
		if ( empty ( $args ) || empty( $args['seller_email'] ) ) {
			die( esc_html_e( 'No seller email address sent to ErrorInRecordNotification', 'gothic-selections' ) );
		}

		$recipients = $args['seller_email'];
	...
	protected static function cc( array $args = [] ) {
		...
		if ( empty ( $args ) || empty( $args['buyer_email'] ) ) {
			die( esc_html_e( 'No email address sent to StepAccessFoundOrder', 'gothic-selections' ) );
		}

		$recipients = $args['buyer_email'];
	...
	protected static function subject( array $args = [] ) {
		...
		$subject = __( 'Gothic Landscape Selections: Your Home Buyer Requested Revisions', 'gothic-selections' );
