<?php
/**
 * Selections Completed (User) Notification Email
 *
 ...
 * @abstract
 */
final class UserCompletedSelectionEmail extends MessageAbstract implements MessageInterface {
	 ...
	 * @var string
	 */
	public static $key = 'completed-for-user';
	...
	public static function message( array $args = [] ) {
		...
		$history = json_decode(get_post_meta( $args["id"], 'order_history', true )) ?? [];
		
		$time = current_datetime();
		$history[] = [
			"history_entry_title" => "User Completed Order",
			"history_entry_date" => $time
		];
		$history = json_encode($history);
		
		update_post_meta( $args["id"], 'order_history',  $history);
	...
	protected static function recipients( array $args = [] ) {
		...
		if ( empty ( $args ) || empty( $args['email'] ) ) {
			die( esc_html_e( 'No seller email address sent to UserCompletedSelectionEmail', 'gothic-selections' ) );
		}
	...
	protected static function subject( array $args = [] ) {
		...
		$subject = __( 'Completed Landscape Selections Order', 'gothic-selections' );
