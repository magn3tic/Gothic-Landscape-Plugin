 ...
 * @abstract
 */
final class UserRemindSelectionEmail extends MessageAbstract implements MessageInterface {
	 ...
	 * @var string
	 */
	public static $key = 'reminder-for-user';
	...
	public static function message( array $args = [] ) {
		...
		$history[] = [
			"history_entry_title" => "Reminder Sent",
	...
	protected static function recipients( array $args = [] ) {
		...
		if ( empty ( $args ) || empty( $args['email'] ) ) {
			die( esc_html_e( 'No email address sent to UserRemindSelectionEmail', 'gothic-selections' ) );
		}
	...
	protected static function subject( array $args = [] ) {
		...
		$subject = __( 'Please Complete Your Landscape Selections Order', 'gothic-selections' );
